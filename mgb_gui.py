...
def file_open():
    ...
    if SearchPrefs.searchtype.get() == "homology":
      location = tkFileDialog.askopenfilename(filetypes=(("GenBank files",('*.gbk','*.gb','*.genbank')),("EMBL files",('*.embl','*.emb'))), initialdir=APPDATA)
      infile.set(location)
      ...
      location = tkFileDialog.askopenfilename(filetypes=(("FASTA files",('*.fasta','*.fas','*.fa')),("All file types",('*.*'))))
...
def db_open():
    global APPDATA
    location = tkFileDialog.askopenfilename(initialdir = APPDATA, filetypes=(("MGB database files",('*.pal','*.nal')),("MGB database files",())))
    if len(location) > 0:
...
class SearchPreferences:
  ...
  def defineSearchType(self, frame, searchtype):
    if searchtype.get() == 1:
      ...
      infile.set("<No file selected>")
      if len(self.text2.grid_info().keys()) == 0:
