...
import webbrowser
import tkFileDialog
from tkMessageBox import askyesno, showerror, showwarning, showinfo
...
class GenBankFileDownload(Frame):
    ...
    def _download(self, event=None):
        ...
        filename = tkFileDialog.asksaveasfilename(defaultextension="gbk", filetypes = [('GenBank files', '.gbk'), ('all files', '.*')])
        #Open 'loading...' message
        loading = Toplevel(self.modalPane, height=200, width=400)
...
class OutputFolderSelectionFrame(Frame):
    ...
    def select(self):
        selected = tkFileDialog.askdirectory(initialdir=self.initialDir, mustexist=False)
...
class MessageBox(object):
    ...
    def sendreport(self, event=None):
      try:
        webbrowser.open("mailto:multigeneblast@gmail.com?SUBJECT=ErrorReport&BODY=" + urllib2.quote(self.errormessage.encode("utf8")))
      except:
...
class MakeDatabase(Frame):
    ...
    def _addfile(self):
        if self.dbtype.getval() == 0:
          filenames = tkFileDialog.askopenfilename(multiple=True, filetypes=(("GenBank files",('*.gbk','*.gb','*.genbank')),("EMBL files",('*.embl','*.emb'))), title="Select files", initialdir=self.directory)
        else:
          filenames = tkFileDialog.askopenfilename(multiple=True, filetypes=(("FASTA files",('*.fasta','*.fa','*.fas')), ("GenBank files",('*.gbk','*.gb','*.genbank')),("EMBL files",('*.embl','*.emb'))), title="Select files", initialdir=self.directory)
        if sys.platform == ('win32'):
          if "} {" in filenames:
