...
def sortdictkeysbyvalues(dict):
    items = [(len(dict[key].keys()), key) for key, value in dict.items()]
    items.sort()
  ...
  for i in fileinput.input(dbname + "_all.txt"):
    ...
    if len(i) > 0:
      if z < 10000:
        ...
        if proteininfodict.has_key(tag):
            if proteininfodict[tag].has_key(protein):
              #fileinput.close()
        ...
        for j in proteininfodict.keys():
          ...
          try:
            ...
            picklefile.close()
            tempdict = dict(proteininfodict[j].items() + tagdict.items())
